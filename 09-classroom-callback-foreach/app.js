...
console.log(convertToString(44))
...
*/
const totalCharacterString = name => name.length
const response = totalCharacterString('wellington pinho')
...
*/
const convertStringToLowerCase = name => name.toLowerCase()
const stringToLowerCase = convertStringToLowerCase('CHOCOTONE E OVO DE PÁSCOA JUNTOS NO MERCADO EM PLENO FEVEREIRO')
...
*/
const returnIdexInString = (character, name) => name.indexOf(character)
const indexInString = returnIdexInString('2', '123')
...
*/
const itemExistCallback = (item, arr) => arr.includes(item)

const itemExist = itemExistCallback(1, [1, 2, 3])
...
*/
const concateArraysFunc = (arr1, arr2) => arr1.concat(arr2)
const concateArrays = concateArraysFunc(
  [{
    name: 'Francisco',
    lastName: 'Wellington'
  }],
...
*/
const removeLastItemFromArray = arr => {
  arr.pop()
  return arr
}
const removedLastIndexFromArray = removeLastItemFromArray([1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 10])
console.log(removedLastIndexFromArray)
/*
  08
  ...
  - Crie uma função que retorna se o valor passado como argumento em sua  
    invocação é null.
*/
const paramsIsNull = value => !value
const paramsIsnotNull = paramsIsNull(null)
console.log(paramsIsnotNull)
/*
  09
  ...
  - Invoque a função que recebe um callback por parâmetro, passando como  
    ...
    foi exibido.
*/
const getResponseCallbackFun = (callback) => {
  callback()
}
getResponseCallbackFun(() => console.log('wellpinho'))
/*
  10
...
*/
const getTripleValue = (value, callback) => {
  return callback(value)
}

const triple = num => num * 3

console.log(getTripleValue(50, triple))
...
review.forEach(renderParagraph)
...
const whoLiked = likePosts(['well', 'mary', 'raica', 'wendell', 'zé'])
